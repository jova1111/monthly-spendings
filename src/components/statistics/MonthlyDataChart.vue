<template>
  <div>
    ...
    <chart class="center-middle" :chart-data="parsedChartData" :options="chartOptions"></chart>
  </div>
</template>
    ...
    methods: {
      ...
      changeGraphMode() {
        if (this.selectedGraphData == this.graphDataModes.MONTHS.selectOption) {
          ...
          this.parsedChartData = this.getGraphData(this.graphDataModes.QUARTALS.value);
        }
      },
